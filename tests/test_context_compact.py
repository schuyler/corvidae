...
class TestContextCompactPlugin:
    ...
    async def test_no_block_below_threshold(self, db, plugin):
        ...
        result = await plugin.compact_conversation(conv, mock_client, max_tokens=10000)
    ...
    async def test_block_generated_above_threshold(self, db, plugin):
    ...
    async def test_block_truncated_to_max_chars(self, db, plugin):
        ...
        await plugin.compact_conversation(conv, mock_client, max_tokens=10000)
    ...
    async def test_block_generation_failure_logged(self, db, plugin):
    ...
    async def test_no_duplicate_blocks(self, db, plugin):
    ...
    async def test_disabled_plugin_noop(self, db, plugin):
    ...
    async def test_injects_background_block(self, db, plugin):
    ...
    async def test_no_injection_without_block(self, db, plugin):
    ...
    async def test_no_injection_when_disabled(self, db, plugin):
    ...
    async def test_tracked_turns_increment(self, db, plugin):
    ...
    async def test_stats_tool_returns_info(self, db, plugin):
    ...
    async def test_stats_tool_disabled(self, db, plugin):
...
class TestContextCompactIntegration:
    ...
    async def test_full_flow_generate_and_inject(self, db):
    ...
    async def test_block_survives_compaction(self, db):
        ...
        await cc_plugin.compact_conversation(conv, mock_client, max_tokens=1000)

        # Then trigger compaction.
        await comp_plugin.compact_conversation(conv, mock_client, max_tokens=1000)
