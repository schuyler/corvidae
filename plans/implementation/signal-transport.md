...
2. On buster-host: install Java 21+ and signal-cli; choose the socket path (a
...
5. Start `signal-cli daemon` with `--receive-mode=on-connection` (or the
   ...
   an unattended reboot of buster-host; corvidae likewise.
...
8. Add the `signal:` block and allowlist (your ACI) to agent.yaml on buster-host;
   restart the daemon.
...
10. Optionally link Signal Desktop to the bot account for the mirror; it
    unlinks after 30 days offline but is re-linkable from buster-host.
11. Confirm remote SSH access to buster-host from the road (R14 observability and
    any surgery).
