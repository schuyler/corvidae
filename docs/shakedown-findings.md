...
`~/code/llm/buster/sessions/<UTC-ts>/` on sagan, which is a disposable,
...
- Before driving a session against a channel that hasn't been exercised
  ...
  *deployed* `state/agent.yaml` on sagan, not just committed in
...
- Query `state/sessions.db` directly (the `sqlite3` CLI is installed on
  sagan) to verify what actually happened — what a summary retained vs.
