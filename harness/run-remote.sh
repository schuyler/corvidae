#!/usr/bin/env bash
# Mac-side wrapper: push a rev to sagan's bare repo, then run the harness
...
ssh sagan "cd ~/code/llm/buster/repo && git fetch origin && git checkout -f $REV_SHA && exec harness/run.sh"
