#!/usr/bin/env bash
# Provision signal-cli on sagan and install the systemd unit that supervises
# its JSON-RPC daemon. Run on sagan as root; idempotent.
#
...
# user corvidae runs as — on sagan the daemon runs under screen as sderle, not
